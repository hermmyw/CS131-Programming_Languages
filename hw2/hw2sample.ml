...
(* let t = (Node (Expr,
     [Node (Term,
      ...
      [Node (Term,
             [Node (Num,
              [Leaf "2"])])])]));;
let flattened_tree = parse_tree_leaves t;;

let parser = make_parser awkish_grammar small_awk_frag;;

let unwrap s = match s with
  | Some x -> x;;

let tree = unwrap parser;;

let x = parse_tree_leaves tree;; *)
